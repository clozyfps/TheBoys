...
import net.mcreator.theboys.procedures.ATrainDisplayProcedure;

import com.mojang.blaze3d.systems.RenderSystem;
import com.mojang.blaze3d.platform.GlStateManager;

@Mod.EventBusSubscriber({Dist.CLIENT})
public class SpeedSkillBarOverlay {
	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (ATrainDisplayProcedure.execute(entity)) {
			if (ATrainDisplayProcedure.execute(entity)) {
				event.getGuiGraphics().blit(new ResourceLocation("the_boys:textures/screens/ability_bar_a_train.png"), 5, h - 31, 0, 0, 97, 25, 97, 25);
			}
			...
			if (SelectedMove3DisplayProcedure.execute(entity)) {
				event.getGuiGraphics().blit(new ResourceLocation("the_boys:textures/screens/ability_selected.png"), 41, h - 32, 0, 0, 26, 26, 26, 26);
			}
			if (SelectedMove4DisplayProcedure.execute(entity)) {
				event.getGuiGraphics().blit(new ResourceLocation("the_boys:textures/screens/ability_selected.png"), 59, h - 32, 0, 0, 26, 26, 26, 26);
			}
			if (SelectedMove5DisplayProcedure.execute(entity)) {
				event.getGuiGraphics().blit(new ResourceLocation("the_boys:textures/screens/ability_selected.png"), 77, h - 32, 0, 0, 26, 26, 26, 26);
			}
		}
