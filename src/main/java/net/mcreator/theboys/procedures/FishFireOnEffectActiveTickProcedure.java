...
public class FishFireOnEffectActiveTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		entity.getPersistentData().putDouble("fishfiretimer", (entity.getPersistentData().getDouble("fishfiretimer") + 1));
		if (entity.getPersistentData().getDouble("fishfiretimer") >= 19) {
			if (!world.getEntitiesOfClass(Cod.class, AABB.ofSize(new Vec3(x, y, z), 25, 25, 25), e -> true).isEmpty()) {
				((Entity) world.getEntitiesOfClass(Cod.class, AABB.ofSize(new Vec3(x, y, z), 25, 25, 25), e -> true).stream().sorted(new Object() {
				...
				TheBoysMod.queueServerWork(10, () -> {
					if (!world.getEntitiesOfClass(Cod.class, AABB.ofSize(new Vec3(x, y, z), 25, 25, 25), e -> true).isEmpty()) {
						if (!((Entity) world.getEntitiesOfClass(Cod.class, AABB.ofSize(new Vec3(x, y, z), 25, 25, 25), e -> true).stream().sorted(new Object() {
							Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
								return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
							}
